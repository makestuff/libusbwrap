ifeq ($(OS),Windows_NT)
	...
	ifeq ($(MACHINE),x64)
		ifeq ($(PLATFORM),mingw)
			LINK_EXTRALIBS_REL := $(ROOT)/3rd/libusbx-$(LIBUSB_VERSION)-win/MinGW64/dll/libusb-1.0.dll
		...
		ifeq ($(PLATFORM),mingw)
			LINK_EXTRALIBS_REL := $(ROOT)/3rd/libusbx-$(LIBUSB_VERSION)-win/MinGW32/dll/libusb-1.0.dll
			EXTRADLLS_REL := $(LINK_EXTRALIBS_REL)
		else
